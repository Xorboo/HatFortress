...
public class MyEntity implements IEntity {
	...
	public MyEntity(final float pX, final float pY) {
		this.mX = pX;
		this.mY = pY;
	}
	...
	public void detachChildren() {
		...
		this.mChildren.clear(MyEntity.PARAMETERCALLABLE_DETACHCHILD);
	}
	...
	public boolean detachChild(final IEntity pEntity) {
		...
		return this.mChildren.remove(pEntity, MyEntity.PARAMETERCALLABLE_DETACHCHILD);
	}
	...
	public IEntity detachChild(final int pTag) {
		...
		for(int i = this.mChildren.size() - 1; i >= 0; i--) {
			if(this.mChildren.get(i).getTag() == pTag) {
				...
				MyEntity.PARAMETERCALLABLE_DETACHCHILD.call(removed);
				return removed;
			}
		}
		return null;
	...
	public IEntity detachChild(final IEntityMatcher pEntityMatcher) {
		...
		return this.mChildren.remove(pEntityMatcher, MyEntity.PARAMETERCALLABLE_DETACHCHILD);
	}
	...
	public boolean detachChildren(final IEntityMatcher pEntityMatcher) {
		...
		return this.mChildren.removeAll(pEntityMatcher, MyEntity.PARAMETERCALLABLE_DETACHCHILD);
	}
	...
	public float[] convertLocalToSceneCoordinates(final float pX, final float pY) {
		return this.convertLocalToSceneCoordinates(pX, pY, MyEntity.VERTICES_LOCAL_TO_SCENE_TMP);
	}
	...
	public float[] convertLocalToSceneCoordinates(final float pX, final float pY, final float[] pReuse) {
		pReuse[Constants.VERTEX_INDEX_X] = getX();
		pReuse[Constants.VERTEX_INDEX_Y] = getY();
	...
	public float[] convertLocalToSceneCoordinates(final float[] pCoordinates) {
		return this.convertLocalToSceneCoordinates(pCoordinates, MyEntity.VERTICES_LOCAL_TO_SCENE_TMP);
	}
	...
	public float[] convertSceneToLocalCoordinates(final float pX, final float pY) {
		return this.convertSceneToLocalCoordinates(pX, pY, MyEntity.VERTICES_SCENE_TO_LOCAL_TMP);
	}
	...
	public float[] convertSceneToLocalCoordinates(final float[] pCoordinates) {
		return this.convertSceneToLocalCoordinates(pCoordinates, MyEntity.VERTICES_SCENE_TO_LOCAL_TMP);
	}
	...
	private void allocateEntityModifiers() {
		this.mEntityModifiers = new EntityModifierList(this, MyEntity.ENTITYMODIFIERS_CAPACITY_DEFAULT);
	}

	private void allocateChildren() {
		this.mChildren = new SmartList<IEntity>(MyEntity.CHILDREN_CAPACITY_DEFAULT);
	}

	private void allocateUpdateHandlers() {
		this.mUpdateHandlers = new UpdateHandlerList(MyEntity.UPDATEHANDLERS_CAPACITY_DEFAULT);
	}
